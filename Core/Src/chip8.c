...
// Source: libgcc
static void *memset_(void *dest, int val, uint64_t len) {
...
// Source: libgcc
static void *memcpy_(void *dest, const void *src, uint64_t len) {
  char *d = dest;
  const char *s = src;
  while (len--)
    *d++ = *s++;
...
bool c8_sound(Chip8 *vm) { return vm->ST > 0; }

bool c8_screen_updated(Chip8 *vm) { return vm->screen_updated; }

bool c8_ended(Chip8 *vm) { return vm->opcode == 0x00FD; }
...
void set_pixel(Chip8 *vm, int row, int col) {
  ASSERT(row >= 0 && row <= 63 && col >= 0 && col <= 127);
  int byte = 128 * row + col;           // Row major order
  byte /= 8;                            // Convert 128x64 to 16x64
...
void clear_pixel(Chip8 *vm, int row, int col) {
  ASSERT(row >= 0 && row <= 63 && col >= 0 && col <= 127);
  int byte = 128 * row + col;           // Row major order
  byte /= 8;                            // Convert 128x64 to 16x64
...
int c8_get_opcode(Chip8 *vm) { return vm->opcode; }
...
void c8_set_platform(Chip8 *vm, Platform plt) { vm->platform = plt; }

void c8_set_is_polling(Chip8 *vm, bool is_polling) {
  vm->is_polling = is_polling;
}
// Display n-byte sprite starting at memory location I at (Vx, Vy),
// set VF = collision
static void op_Dxyn(Chip8 *vm, uint8_t x, uint8_t y, uint8_t n) {
  vm->V[0xF] = 0;
...
static void op_Dxy0(Chip8 *vm, uint8_t x, uint8_t y, uint8_t n) {
  ...
  for (int row = 0; row < 16; row++) {
    if (yo + row >= screen_height)
      break;
    uint16_t sprite_row =
        vm->RAM[vm->I + 2 * row] << 8 | vm->RAM[vm->I + 2 * row + 1];
    ...
    for (int col = 0; col < 16; col++) {
      if (xo + col >= screen_width)
        break;
      int xc = xo + col; // X origin + X offsetchip
      int yc = yo + row; // Y origin + Y offset
...
static void op_00Cn(Chip8 *vm, uint8_t n) {
...
static void op_00FB(Chip8 *vm) {
  int rows = vm->hi_res ? 64 : 32;
  int cols = vm->hi_res ? (128 / 8) : (64 / 8);
  for (int y = 0; y < rows; y++) {
    for (int x = cols - 1; x > 0; x--) {
      vm->screen[y * cols + x] =
          vm->screen[y * cols + x] >> 4 | vm->screen[y * cols + x - 1] << 4;
    }
    vm->screen[y * cols] >>= 4;
  }
}
...
static void op_Fx0A(Chip8 *vm, uint8_t x) {
...
static int decode_and_execute(Chip8 *vm) {
  // Decode
